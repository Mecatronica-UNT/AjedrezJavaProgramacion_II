...
public class VentanaTablero {
    ...
    private final PanelHistorialJuego panelHistorialJuego;
    private final VentanaPiezasTomadas ventanaPiezasTomadas;
    private final PanelTablero panelTablero;
    private final MovRegistro movRegistro;
    private Tablero AjedrezTablero;
    private DirecciónTablero direccionTablero;
    private Boolean mostrarMovimientosLegales;
    ...
    private final static Dimension DimensionTablero = new Dimension(400,400);
    ...
    public VentanaTablero(){
        ...
        this.ventanaPiezasTomadas = new VentanaPiezasTomadas();
        this.panelHistorialJuego = new PanelHistorialJuego();
        this.panelTablero = new PanelTablero();
        this.movRegistro = new MovRegistro();
        this.direccionTablero = DirecciónTablero.NORMAL;
        this.mostrarMovimientosLegales = false;
        this.FrameJuego.add(this.ventanaPiezasTomadas, BorderLayout.WEST);
        this.FrameJuego.add(this.panelHistorialJuego, BorderLayout.EAST);
    ...
    private JMenu crearArchivoMenu() {
        ...
        salirMenuItem.addActionListener((final ActionEvent e) -> {
            System.exit(0);
        });
    ...
    private JMenu crearMenuPreferencias(){
        ...
        final JMenuItem itemVoltearTablero = new JMenuItem("Invertir Tablero");
        itemVoltearTablero.addActionListener((final ActionEvent e) -> {
        ...
        menuPreferencia.addSeparator();
        final JCheckBoxMenuItem mostrarMovimientosLegalesCheckBox = new JCheckBoxMenuItem("Mostrar Movimientos Legales", false);
        mostrarMovimientosLegalesCheckBox.addActionListener((ActionEvent e) -> {
            mostrarMovimientosLegales = mostrarMovimientosLegalesCheckBox.isSelected();
        });
        menuPreferencia.add(mostrarMovimientosLegalesCheckBox);
    ...
    private class PanelTablero extends JPanel {
        ...
        public void dibujarTablero(final Tablero tablero){
            removeAll();
            direccionTablero.transverso(Casillastablero).stream().map((casillaPanel) -> {
                casillaPanel.dibujarCasilla(tablero);
                return casillaPanel;
            }).forEachOrdered((casillaPanel) -> {
                add(casillaPanel);
            });
            validate();
            repaint();
    ...
    public static class MovRegistro{
        ...
        public void añadirMovimiento(final Movimiento movimiento){
        ...
        public Movimiento removerMovimiento(int index){
            return this.movimientos.remove(index);
        }
        
        public boolean removerMovimiento(Movimiento movimiento){
    ...
    private class PanelCasilla extends JPanel {
                    ...
                    if(isRightMouseButton(e)){
                        ...
                        if(casillaFuente == null){
                            ...
                            if(transición.getEstatusDeMovimiento().estáHecho()){
                                ...
                                movRegistro.añadirMovimiento(movimiento);
                            }
                         ...
                         SwingUtilities.invokeLater(() -> {
                             ventanaPiezasTomadas.rehacer(movRegistro);
                             panelHistorialJuego.rehacer(AjedrezTablero, movRegistro);
                             panelTablero.dibujarTablero(AjedrezTablero);
                        });
                    }
                }
        ...
        private void mostrarMovimientosLegales(final Tablero tablero){
            if(mostrarMovimientosLegales){
                movimientosLegalesPieza(tablero).stream().filter((movimiento) -> (movimiento.getCoordenadaDeDestino() == this.casillaID)).forEachOrdered((_item) -> {
                    try{
                        final BufferedImage imagen=
                            ImageIO.read(new File("C:\\Users\\ASUS\\Documents\\NetBeansProjects\\AjedrezJava\\src\\main\\java\\art\\marco.png"));
                        add(new JLabel(new ImageIcon(imagen)));
                    } catch(Exception e){
                        e.printStackTrace();
                    }
        ...
        private Collection<Movimiento> movimientosLegalesPieza(final Tablero tablero){
            if(PiezaMovHumano != null && PiezaMovHumano.getColorPieza() == tablero.jugadorActual().getColor()){
                return PiezaMovHumano.calcularMovimientosLegales(tablero);
            }
